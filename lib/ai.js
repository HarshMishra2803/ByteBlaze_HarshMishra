  ...
  const myTurn = queuePromise.then(async () => {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        await sleep(2000); // Rate limiter space
        return text;
      } catch (error) {
...
async function callGroq(messages) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 429) throw new Error('429 Too Many Requests: ' + errorMsg);
    if (response.status >= 500) throw new Error('503 Server Error: ' + errorMsg);
    throw new Error(`Groq API Error (${response.status}): ${errorMsg}`);
  }

  if (!data || !data.choices || !data.choices[0] || !data.choices[0].message) {
    throw new Error('Invalid response form from AI API');
  }
